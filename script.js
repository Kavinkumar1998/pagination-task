...
nextbtn.addEventListener("click",()=>{
    ...
    var data = userdata.length;
    var ittreatevalue= Number(data/10)*10;
    for(var i=1;i<=10;i++){
        var takingbtn = document.querySelector(`#btn-${i}`)
        var attributevalue = Number(takingbtn.getAttribute('for'))
    if(ittreatevalue>=attributevalue){
    var increasingaatributevalue = attributevalue + 10;
    takingbtn.setAttribute('for', increasingaatributevalue)
    takingbtn.innerText = increasingaatributevalue
...
previousbtn.addEventListener('click', ()=>{
    ...
    for(var i=1;i<=10;i++){
        ...
        var decreasingaatributevalue = attributevalue - 10
        takingbtn.setAttribute('for', decreasingaatributevalue)
takingbtn.innerText = decreasingaatributevalue
   }
     }
})

function byclicking(e){
    document.querySelector('.outdata').innerHTML=''
        var for_number = parseInt(e.getAttribute('for')) -1;
       var res_data = userdata[for_number]
       var create_div =document.createElement("div")
        create_div.innerHTML = `
         <p for = ${for_number}>ID: ${res_data.id}</p>
         <p for = "name">Name: ${res_data.name}</p>
         <p for = "email">Email: ${res_data.email}</p>
        `
        document.querySelector('.outdata').append(create_div)

}
function byclickingfirst(e){
    document.querySelector('.outdata').innerHTML=''
        var for_number = 1 -1;
       var res_data = userdata[for_number]
       var create_div =document.createElement("div")
...
function byclickinglast(e){
    document.querySelector('.outdata').innerHTML=''
        var for_number = userdata.length -1;
       var res_data = userdata[for_number]
       var create_div =document.createElement("div")
        create_div.innerHTML = `
         <p for = ${for_number}>ID: ${res_data.id}</p>
